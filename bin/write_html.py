...
def write_html(run_id: str, lpo_lco_ldo: str, files: list):
    ...
    with open(f"{lpo_lco_ldo}.html", "w") as f:
        ...
        corr_files = [f for f in files if lpo_lco_ldo in f and f.startswith("corr_comp_scatter") and f.endswith("drug.html")]
        write_scatter_eval_models(f=f, setting=lpo_lco_ldo, group_by="drug", plot_list=corr_files)
        corr_files = [f for f in files if lpo_lco_ldo in f and f.startswith("corr_comp_scatter") and f.endswith("cell_line.html")]
